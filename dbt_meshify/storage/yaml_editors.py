...
class DbtMeshModelYmlEditor:
    ...
    def process_model_yml(self, model_yml: Dict):
        ...
        return {k: v for k, v in model_ordered_dict.items() if v is not None}

    def add_model_contract_to_yml(
        self, model_name: str, model_catalog: Optional[CatalogTable], full_yml_dict: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Adds a model contract to the model's yaml"""
        # set up yml order

        # parse the yml file into a dictionary with model names as keys
        yaml_models: List[Dict[str, Any]] = full_yml_dict.get("models", [])
        models = {model["name"]: model for model in yaml_models}
        model_yml = models.get(model_name) or {"name": model_name, "columns": [], "config": {}}
...
class DbtMeshModelConstructor(DbtMeshModelYmlEditor):
    ...
    def add_model_contract(self) -> None:
        ...
        if not yml_path:
            if not node.original_file_path:
                raise Exception("No identifiable path to the model's YAML properties found.")

            yml_path = Path(node.original_file_path).parent / "_models.yml"
        ...
        updated_yml = self.add_model_contract_to_yml(
            ...
            full_yml_dict=full_yml_dict,  # type: ignore
        )
    ...
    def add_model_version(
        ...
        if not node.original_file_path:
            raise Exception(
                f"The SQL file that defines {self.model_node.name} could not be found "
            )
        ...
        next_version_file_name = (
            ...
            if defined_in
            else f"{node.name}_v{int(latest_version) + 1}.{node.language}"
        )
